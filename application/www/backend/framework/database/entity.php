...
abstract class Entity
{
    ...
    public function save()
    {
        ...
        if (!$this->arePrimaryKeysFilled() && count($this->getPrimaryKeys()) == 1)
        {
            ...
            $keyValue = $row->getValues($types);
            foreach ($this->getPrimaryKeys() as $key)
            {
                $this->{$key} = $keyValue['id'];
            }
        }
        else
        {
    ...
    protected function getSelectQuery($forUpdate = null)
    {
        ...
        {
            if (isset($types[$value]) && ($types[$value] == 'string'))
            {
                // Do a case-sensitive comparision for strings. Use istrings for case-insensitive matching.
                return $value . ' = :' . $value . ' COLLATE utf8_bin';
            }
            else
            {
                return $value . ' = :' . $value;
     ...
     * 
     * @param $returning If true, the query will return the inserted Id. This is only
     *                   valid for single primary keys.
     *
